...
class WulpusConnection:
    ...
    def __init__(self, type='dongle'):
        ...
        if type == 'dongle':
            ...
            self.__connection__ = WulpusDirect()
        else:
    ...
    def get_available(self) -> list:
        future = asyncio.run_coroutine_threadsafe(self.__connection__.get_available(), self.loop)
        return future.result()
    
    def open(self, device) -> bool:
        self.device = device
        future = asyncio.run_coroutine_threadsafe(self.__connection__.open(device), self.loop)
        return future.result()
    
    def close(self) -> bool:
        future = asyncio.run_coroutine_threadsafe(self.__connection__.close(), self.loop)
        return future.result()
    
    def send_config(self, conf_bytes_pack:bytes) -> bool:
        self.conf_bytes_pack = conf_bytes_pack
        future = asyncio.run_coroutine_threadsafe(self.__connection__.send_config(conf_bytes_pack), self.loop)
